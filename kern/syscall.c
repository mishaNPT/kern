...
sys_map_region(envid_t srcenvid, uintptr_t srcva,
    ...
    // LAB 9: Your code here
    struct Env* srcenv;
    if (envid2env(srcenvid, &srcenv, 1)) return -E_BAD_ENV;

    struct Env* dstenv;
    if (envid2env(dstenvid, &dstenv, 1)) return -E_BAD_ENV;
    ...
    return map_region(&dstenv->address_space, dstva, &srcenv->address_space, srcva, size, perm) ? -E_NO_MEM : 0;
}
...
sys_unmap_region(envid_t envid, uintptr_t va, size_t size) {
    ...
    struct Env* env;
    if (envid2env(envid, &env, 1)) return -E_BAD_ENV;
    if (CLASS_MASK(0) & va || va >= MAX_USER_ADDRESS) return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, uintptr_t srcva, size_t size, int perm) {
    ...
    if (srcva < MAX_USER_ADDRESS && env->env_ipc_dstva < MAX_USER_ADDRESS) {
        if (PAGE_OFFSET(srcva) || PAGE_OFFSET(env->env_ipc_dstva) || perm & ~PROT_ALL ||
            (perm & ~(PTE_AVAIL | PTE_W)) != (PTE_U | PTE_P))
            return -E_INVAL;

        if (map_region(&env->address_space, env->env_ipc_dstva, &curenv->address_space, srcva, PAGE_SIZE, perm | PROT_USER_))
            return -E_NO_MEM;
...
sys_ipc_recv(uintptr_t dstva, uintptr_t maxsize) {
    // LAB 9: Your code here
    if (PAGE_OFFSET(maxsize) || (dstva < MAX_USER_ADDRESS && (PAGE_OFFSET(dstva) || maxsize == 0))) 
        return -E_INVAL;
